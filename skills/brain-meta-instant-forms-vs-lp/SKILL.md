...
description: Use when the user asks "instant form or landing page", "why are my Meta leads junk", "More Volume or Higher Intent", "should I use OTP", "how do I filter lead quality", "connect Facebook lead forms to your CRM", "conditional logic", or "cost per lead vs closed deal". Landing-page CRO is covered by `brain-paid-traffic-landing-pages-leadgen` (in the optional extras pack of this kit).
metadata:
  type: expert-brain
...
- **Copy and creative pre-filter quality before the form loads.** Umar Tazkeer (Apply Now vs Learn More, and *price in headline* — see the caveat below), Dr. Matt Shiver ("what you say in the ads is the most important part"), Cardinal (encouragement angle 25% lower CPL). Kill Audience Network on lead campaigns (Smart Marketing Zone: "very cheap leads... wastes our budget"). **Caveat on price-in-headline:** if your own policy keeps price off ad copy, use the other three pre-filters instead of Tazkeer's price-in-headline lever. Pre-filter with the other levers instead: an "Apply"-class CTA rather than "Learn more", a qualifying question before the contact fields, and copy specific enough that the wrong person self-selects out. This is the same ruling stated in `brain-paid-traffic-landing-pages-leadgen` (in the optional extras pack of this kit), which owns the conflict.
...
- IF the offer is under about $2,000 THEN use an instant form with a qualifying question; IF it is higher-ticket or considered (needs a VSL, education or social proof) THEN use a landing page, not a form (Zwolinski, Gavin Wiener).
...
- Minimum 2-3 days per test cell (Umar Tazkeer) — that is the *floor* for a cell to be worth looking at, not a verdict window. **The verdict rule is the verdict gate: no kill or scale call below $100 spend + 7 days + 3 conversions**, which also governs Tazkeer's permutation matrix (his 60-day arithmetic gets longer, not shorter, once each cell has to clear the gate — which is exactly why the 3x3 matrix exists instead of testing everything).
...
**One rule: instant form with a qualifying question for offers under about $2,000; landing page for higher-ticket or considered offers.** The landing-page funnel is **ad → landing page (form on the page) → pixel Lead fires on submit**. At higher price points instant forms tend to produce lead volume without buyers unless they carry a qualifying question, a higher-intent form type, an age cap and a human follow-up lane. Forms remain the right tool for lower-ticket volume plays with a qualifying question, the Higher-Intent form type and a human follow-up lane.
...
2. **Run one controlled test: Higher-Intent form vs the page, judged on cost per closed deal, not cost per lead.** A more expensive lead that closes three times as often wins. Verdicts only past the verdict gate ($100 spend + 7 days + 3 conversions).
...
- **Price in the headline as a pre-qualifier (Umar Tazkeer).** If your own policy keeps price off ad copy, use the other three pre-filters. Pre-filter with the CTA wording, the qualifying question and copy specificity instead. `brain-paid-traffic-landing-pages-leadgen` (in the optional extras pack of this kit) covers the page side; note that keeping price off the ad also weakens the case for a price anchor in the page hero, since the ad cannot set it up.
...
- `brain-meta-emq-and-match-quality` (in the optional extras pack of this kit), Meta event match quality and advanced matching (EMQ)

## Pairs with / boundaries

- `brain-paid-traffic-landing-pages-leadgen` (in the optional extras pack of this kit) owns the LP side in depth (page structure, CRO, VSL, hero/message-match). This brain covers LPs only as the OTHER side of the forms-vs-LP decision; go there for building or optimising the page itself.
- `brain-meta-optimisation-event-strategy` (in the optional extras pack of this kit) owns the lead-quality signal and event-optimisation doctrine. This brain references it (Theme 8) but does not duplicate the CAPI/event-selection deep dive.
- `brain-meta-ads-manual-control-no-advantage` (in the optional extras pack of this kit) owns the account-wide manual-control settings and the Advantage+ kill-list. When a source's audience-automation advice conflicts, that brain wins.
