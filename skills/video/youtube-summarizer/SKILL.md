...
description: "Summarizes a YouTube video from its transcript - metadata, thesis, key insights, takeaway - and saves the full transcript to a file. Use when the user pastes a YouTube link or asks for a video summarized, transcribed or repurposed."
version: 2.0.0
...
Automatically fetch transcripts from YouTube videos, generate structured summaries, and save full transcripts to a file.
...
4. **Full transcript saved** - Saves a timestamped transcript to a file
...
**Required (third-party):** the open-source MCP YouTube Transcript server (kimtaeyoon83/mcp-server-youtube-transcript) must be installed at:
...
Send the structured summary as your response to the user and tell them where the transcript file was saved.
...
**Built on the third-party MCP YouTube Transcript server (Android emulation for cloud reliability).**
