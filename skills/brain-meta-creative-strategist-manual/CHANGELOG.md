# Changelog — brain-meta-creative-strategist-manual

## 2026-08-26 — Advantage+ Audience re-correction (cross-check 2026-08-26)
- **"Switch to original audience is gone" reversed.** The toggle exists and binds at the API layer: `advantage_audience` 0 and 1 both accepted on `OUTCOME_LEADS`, verified live 2026-08-25. What is true: ON fixes `age_max` at 65 (error 1870189) and includes degrade to suggestions; OFF makes includes bind (warm-list pattern).

## 2026-07-28 — currency, methodology + publishing pass
- **NEW: "Creative testing methodology" section.** Concepts per round (4-6 at small budget, not 6-10), the brief→test→read→iterate→retire lifecycle, the ~1-in-10 hit rate set as an expectation, fatigue triggers (frequency 2.5/3.0/3.5/4.0 plus CTR-decay and CPL-drift bands), refresh cadence, and a naming convention that makes a winner attributable. Includes **"How to read a round where nothing wins"** — the most common outcome at $50-90/day, previously addressed nowhere in the library — and a **noise warning**: at ~4 leads/day a 3-conversion gate is under a day of data, so it is a fair-look floor, not a winner threshold.
...
## 2026-07-01 — v1.0
- Built from the existing practitioner corpus (6 ad brains + brain-meta-ads-manual-control-no-advantage 28 sources).
- Angle split: media-buyer (structure/testing/scaling/math) vs creative-strategist (creative-is-the-targeting).
- Hard doctrine: manual control, no Andromeda / Advantage+ AI. Cross-linked companion + shared evidence base.
