# Exact excerpts from retained transcripts

Each short excerpt below is an exact substring of its retained UTF-8 text file.
Offsets are zero-based Python character positions, start inclusive and end exclusive;
they are not byte offsets or video timestamps. Spelling is unchanged, including ASR
artifacts. A local text match is not verified recording accuracy.

Uploader labels and video titles below come from public YouTube oEmbed metadata
retrieved on 2026-09-06. They identify the publishing channel, not necessarily the
person speaking. Verify the recording before treating a transcript excerpt as an
accurate spoken quotation. Context notes are newly authored interpretation.
...
> strong keyframe with a simple prompt will almost always outperform a perfect prompt
...
## Camera description with an existing frame

> All I described was the camera shot, right? How the camera moves
...
> Every prompt is a fresh generation.
...
> C dance copies them as a literal frame into the video output.
...
> create this at least five to 10 times to actually nail the clip
